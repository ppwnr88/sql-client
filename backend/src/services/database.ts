...
export async function listMetadata(
  ...
  switch (config.type) {
    case 'mysql':
      return listMysqlMetadata(config, resource, schema, table);
    case 'postgresql':
      return listPostgresMetadata(config, resource, schema, table);
    case 'mssql':
      return listMssqlMetadata(config, resource, schema, table);
    default:
      throw new Error(`Unsupported database type: ${config.type}`);
  }
}

async function listMysqlMetadata(
  config: DatabaseConfig,
  resource: MetadataResource,
  _schema?: string,
  table?: string
  ...
  try {
    if (resource === 'schemas') {
      return [{ name: config.database ?? '' }].filter((item) => item.name);
    }
    if (resource === 'tables') {
    ...
    return (rows as Array<Record<string, unknown>>).map(normalizeColumn);
  } finally {
    await connection.end();
  }
}

async function listPostgresMetadata(
  config: DatabaseConfig,
  resource: MetadataResource,
  schema?: string,
  table?: string
  ...
  try {
    ...
    return result.rows.map(normalizeColumn);
  } finally {
    await client.end();
  }
}

async function listMssqlMetadata(
  config: DatabaseConfig,
  resource: MetadataResource,
  schema?: string,
  table?: string
  ...
  try {
    ...
    return result.recordset.map(normalizeColumn);
  } finally {
    await pool.close();
  }
}

function normalizeColumn(row: Record<string, unknown>): ColumnMetadata {
  return {
